...
//we cannot directly put middleware so we have to use uploads.single or uploads.any or any other method and pass the field that we have puted in userSchema
...
router.post("/array", uploads.array("profilePic", 3), async (req, res) => {
  try {
...
router.post("/any", uploads.array("profilePic"), async (req, res) => {
  try {
    const filePaths = req.files.map((file) => file.path);
